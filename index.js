...
const CONFIG = {
    visual: {
        ...
        originalWordDisplay: getConfig("lyrics-plus:visual:originalWordDisplay", false),
    ...
    providers: {
        musixmatch: {
            ...
            desc: "与 Spotify 完全兼容。需要一个可以从官方 Musixmatch 应用程序检索的令牌。如果您在检索歌词时遇到问题，请尝试通过单击刷新令牌 <code>刷新令牌</code> 按键.",
        ...
        genius: {
            ...
            desc: "提供不同步的歌词以及艺术家本人的见解。 Genius 已被禁用，无法用作<code>1.2.31</code> 及更高.",
            modes: [GENIUS]
        },
        local: {
            on: getConfig("lyrics-plus:provider:local:on"),
            desc: "提供从以前的 Spotify 会话加载的缓存/本地文件中的歌词.",
            modes: [KARAOKE, SYNCED, UNSYNCED]
        }
    },
    ...
    translate: {
        baidu: {
            on: getConfig("lyrics-plus:translate:baidu:on"),
            desc: "百度翻译 APP_ID 和 SECURITY_KEY 输入格式：APP_ID:SECURITY_KEY",
            token: localStorage.getItem("lyrics-plus:translate:baidu:token")
        }
    },
    modes: ["karaoke", "synced", "unsynced", "genius"],
    modesZh: ["卡拉OK", "同步", "不同步", "genius"],
...
const emptyState = {
    karaoke: null, synced: null, unsynced: null, genius: null, genius2: null, currentLyrics: null
};
...
class LyricsContainer extends react.Component {
    ...
    async fetchColors(uri) {
        ...
        this.setState({
            colors: {
                background: Utils.convertIntToRGB(vibrant), inactive: Utils.convertIntToRGB(vibrant, 3)
            }
        });
    }
    ...
    async tryServices(trackInfo, mode = -1) {
        ...
        for (const id of CONFIG.providersOrder) {
            ...
            if (data.provider !== "local" && finalData.provider && finalData.provider !== data.provider) {
                ...
                finalData.copyright = `${styledMode} 歌词提供者 ${data.provider}\n${finalData.copyright || ""}`.trim();
            }
    ...
    async translateLyrics(silent = true) {
        ...
        for (const params of [["romaji", "spaced", "romaji"], ["hiragana", "furigana", "furigana"], ["hiragana", "normal", "hiragana"], ["katakana", "normal", "katakana"]]) {
            ...
            Promise.all(lyrics.map(lyric => this.translator.romajifyText(lyric.text, params[0], params[1]))).then(results => {
                ...
                Utils.processTranslatedLyrics(result, lyrics, {
                    state: this.state, stateName: params[2]
                });
        ...
        for (const params of [["cn", "hk"], ["cn", "tw"], ["t", "cn"], ["t", "hk"], ["t", "tw"]]) {
    ...
    async onVersionChange(items, index) {
        if (this.state.mode === GENIUS) {
            this.setState({
                ...emptyLine, genius2: this.state.genius2, isLoading: true
            });
            const lyrics = await ProviderGenius.fetchLyricsVersion(items, index);
            this.setState({
                genius: lyrics, versionIndex: index, isLoading: false
            });
        }
    }
    ...
    async onVersionChange2(items, index) {
        if (this.state.mode === GENIUS) {
            this.setState({
                ...emptyLine, genius: this.state.genius, isLoading: true
            });
            const lyrics = await ProviderGenius.fetchLyricsVersion(items, index);
            this.setState({
                genius2: lyrics, versionIndex2: index, isLoading: false
            });
        }
    }
    ...
    processLyricsFromFile(event) {
        ...
        if (file[0].size > 1024 * 1024) {
            Spicetify.showNotification("文件过大", true);
            return;
        }
        ...
        reader.onload = e => {
            try {
                ...
                if (!parsedKeys.length) {
                    Spicetify.showNotification("没有要加载的内容", true);
                    return;
                }
                ...
                Spicetify.showNotification(`已加载 ${parsedKeys.join(", ")} 文件中的歌词`);
            } catch (e) {
                console.error(e);
                Spicetify.showNotification("歌词加载失败", true);
            }
        };

        reader.onerror = e => {
            console.error(e);
            Spicetify.showNotification("读取文件失败", true);
        };
    ...
    componentDidMount() {
        ...
        this.viewPort = document.querySelector(".Root__main-view .os-viewport") ?? document.querySelector(".Root__main-view .main-view-container__scroll-node");

        this.configButton = new Spicetify.Menu.Item("歌词配置", false, openConfig, "lyrics");
    ...
    updateVisualOnConfigChange() {
        this.availableModes = CONFIG.modesZh.filter((_, id) => {
    ...
    componentDidUpdate() {
        ...
        if (this.translationProvider !== CONFIG.visual["translate:translated-lyrics-source"] || this.languageOverride !== CONFIG.visual["translate:detect-language-override"] || this.translate !== CONFIG.visual.translate) {
            this.translationProvider = CONFIG.visual["translate:translated-lyrics-source"];
            this.languageOverride = CONFIG.visual["translate:detect-language-override"];
            this.translate = CONFIG.visual.translate;
    ...
    render() {
        ...
        const out = react.createElement("div", {
            ...
            friendlyLanguage, hasTranslation: {
                musixmatch: this.state.musixmatchTranslation !== null,
                netease: this.state.neteaseTranslation !== null,
                baidu: this.state.baiduTranslation !== null
            }
        }), react.createElement(AdjustmentsMenu, {mode}), react.createElement(Spicetify.ReactComponent.TooltipWrapper, {
            label: this.state.isCached ? "歌词已缓存" : "缓存歌词"
        }, react.createElement("button", {
            className: "lyrics-config-button", onClick: () => {
                const {synced, unsynced, karaoke, genius} = this.state;
                if (!synced && !unsynced && !karaoke && !genius) {
                    Spicetify.showNotification("没有歌词可缓存", true);
                    return;
                }

                this.saveLocalLyrics(this.currentTrackUri, {synced, unsynced, karaoke, genius});
                Spicetify.showNotification("歌词已缓存");
            }
        }, react.createElement("svg", {
            width: 16, height: 16, viewBox: "0 0 16 16", fill: "currentColor", dangerouslySetInnerHTML: {
                __html: Spicetify.SVGIcons[this.state.isCached ? "downloaded" : "download"]
            }
        }))), react.createElement(Spicetify.ReactComponent.TooltipWrapper, {
            label: "从文件加载歌词"
        }, react.createElement("button", {
            className: "lyrics-config-button", onClick: () => {
        ...
        })))), activeItem, !!document.querySelector(".main-topBar-topbarContentWrapper") && react.createElement(TopBarContent, {
            links: this.availableModes,
            activeLink: CONFIG.modesZh[mode],
            lockLink: CONFIG.modesZh[this.state.lockMode],
            switchCallback: label => {
                const mode = CONFIG.modesZh.findIndex(a => a === label);

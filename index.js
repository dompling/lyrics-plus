...
const CONFIG = {
  visual: {
    ...
    "background-color":
      localStorage.getItem("lyrics-plus:visual:background-color") ||
      "var(--spice-main)",
    "active-color":
      localStorage.getItem("lyrics-plus:visual:active-color") ||
      "var(--spice-text)",
    "inactive-color":
      localStorage.getItem("lyrics-plus:visual:inactive-color") ||
      "rgba(var(--spice-rgb-subtext),0.5)",
    "highlight-color":
      localStorage.getItem("lyrics-plus:visual:highlight-color") ||
      "var(--spice-button)",
    alignment: localStorage.getItem("lyrics-plus:visual:alignment") || "center",
    "lines-before":
      localStorage.getItem("lyrics-plus:visual:lines-before") || "0",
    "lines-after":
    ...
    "translation-mode:korean":
      localStorage.getItem("lyrics-plus:visual:translation-mode:korean") ||
      "hangul",
    "translation-mode:chinese":
      localStorage.getItem("lyrics-plus:visual:translation-mode:chinese") ||
      "cn",
    translate: getConfig("lyrics-plus:visual:translate", false),
    originalWordDisplay: getConfig(
      "lyrics-plus:visual:originalWordDisplay",
      false
    ),
    "ja-detect-threshold":
      localStorage.getItem("lyrics-plus:visual:ja-detect-threshold") || "40",
    ...
    "fullscreen-key":
      localStorage.getItem("lyrics-plus:visual:fullscreen-key") || "f12",
    "synced-compact": getConfig("lyrics-plus:visual:synced-compact"),
    "dual-genius": getConfig("lyrics-plus:visual:dual-genius"),
    "global-delay":
      Number(localStorage.getItem("lyrics-plus:visual:global-delay")) || 0,
    delay: 0,
  },
  providers: {
    musixmatch: {
    ...
    genius: {
      on:
        spotifyVersion >= "1.2.31"
          ? false
          : getConfig("lyrics-plus:provider:genius:on"),
...
class LyricsContainer extends react.Component {
  ...
  async tryServices(trackInfo, mode = -1) {
    ...
    for (const id of CONFIG.providersOrder) {
      ...
      if (
        data.error ||
        (!data.karaoke && !data.synced && !data.unsynced && !data.genius)
      )
        continue;
      if (mode === -1) {
        finalData = data;
        ...
        finalData.musixmatchTranslation = finalData.synced.map((line) => ({
          ...
          text:
            finalData.musixmatchTranslation.find(
              (l) =>
                Utils.processLyrics(l.originalText) ===
                Utils.processLyrics(line.text)
            )?.text ?? line.text,
        }));
      }
        ...
        this.setState({ ...CACHE[info.uri], isCached });
        {
          ...
          if (lyricsState) {
            this.state.currentLyrics =
              this.state[CONFIG.visual["translate:translated-lyrics-source"]] ??
              lyricsState;
          }
        }
  ...
  async translateLyrics(silent = true) {
    ...
    for (const params of [
      ...
      Promise.all(
        lyrics.map((lyric) =>
          this.translator.romajifyText(lyric.text, params[0], params[1])
        )
      ).then((results) => {
    ...
    for (const params of [
      ...
      Promise.all(
        lyrics.map((lyric) =>
          this.translator.convertToRomaja(lyric.text, params[1])
        )
      ).then((results) => {
        const result = results.join("\n");
        Utils.processTranslatedLyrics(result, lyrics, {
    ...
    for (const params of [
      ...
      if (
        ...
        (language === "zh-hans" && params[0] === "t") ||
        (language === "zh-hant" && params[0] === "cn")
      )
        continue;
      Promise.all(
        lyrics.map((lyric) =>
          this.translator.convertChinese(lyric.text, params[0], params[1])
        )
      ).then((results) => {
        const result = results.join("\n");
        Utils.processTranslatedLyrics(result, lyrics, {
  ...
  saveLocalLyrics(uri, lyrics) {
    ...
    const localLyrics =
      JSON.parse(localStorage.getItem(`${APP_NAME}:local-lyrics`)) || {};
  ...
  render() {
    const fadLyricsContainer = document.getElementById(
    ...
    if (mode !== -1) {
      ...
      let lyrics =
        CONFIG.visual.translate && translatedLyrics
          ? translatedLyrics
          : this.state.currentLyrics;

      const translatedLyricsCache =
        this.state[CONFIG.visual["translate:translated-lyrics-source"]];

      if (
        CONFIG.visual.originalWordDisplay &&
        translatedLyricsCache &&
        lyrics
      ) {
        lyrics = Utils.processTranslatedOriginalLyrics(
          lyrics,
          this.state.synced
        );
      }

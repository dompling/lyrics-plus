const Utils = {
  ...
  detectLanguage(lyrics) {
    ...
    return ((simpPercentage - tradPercentage + 1) / 2) * 100 >=
  ...
  processTranslatedOriginalLyrics(lyrics, synced) {
    ...
    function setProperty(key, value) {
      dataSouce[key] = { ...dataSouce[key], ...value };
      if (!dataSouceKey.includes(key)) dataSouceKey.push(key);
    }

    lyrics?.forEach((item) => {
      setProperty(item.startTime, { translate: item.text });
    });

    synced?.forEach((item) => {
      setProperty(item.startTime, { text: item.text });
    });

    dataSouceKey.forEach((time) => {
    ...
    return lyrics
      .map((line) => {
        ...
        return `[${this.formatTime(
          line.startTime
        )}]${this.formatTextWithTimestamps(line.text, line.startTime)}`;
      })
      .join("\n");
  },
